...
class Database
{
    ...
    public function connect()
    {
        if(!$this->con)
        {
            ...
            if($myconn)
            {
                $seldb = @mysql_select_db($this->teste_php,$myconn);
                if($seldb)
                {
                    $this->con = true;
                    return true;
    ...
    * to null
    */
    public function setDatabase($teste_php)
    {
        if($this->con)
        {
            if(@mysql_close())
    ...
    * queries
    */
    private function tableExists($contacts)
    {
        $tablesInDb = @mysql_query('SHOW TABLES FROM '.$this->db_name.' LIKE "'.$contacts.'"');
        if($tablesInDb)
        {
    ...
    */
    public function select($contacts, $rows = '*', $where = null, $order = null)
    {
        $q = 'SELECT '.$rows.' FROM '.$contacts;
        if($where != null)
            $q .= ' WHERE '.$where;
        if($order != null)
            $q .= ' ORDER BY '.$order;
    ...
    */
    public function insert($contacts,$values,$rows = null)
    {
    ...
    */
    public function delete($contacts,$where = null)
    {
        if($this->tableExists($contacts))
        {
            if($where == null)
            ...
            {
                $delete = 'DELETE FROM '.$contacts.' WHERE '.$where;
            }
     ...
     */
    public function update($contacts,$rows,$where)
    {
        if($this->tableExists($contacts))
        {
